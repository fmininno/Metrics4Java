...
public class JasomeTool extends ToolAnalyzer {
	...
	public void calculateMetrics() {
			...
			try {				
				...
				System.out.println("ERROR : " + e);
		...
		}else { if(!controlSubFolder(files[0]) && files[0].list().length > 0 && controlJavaFiles(files[0])) {
					try {
					...
					{ 	count++;
						 ...
						 }else { if (error==false) { // if there is a file !=.java extension
	...
	public boolean controlSubFolder(File dir) {
		 for (final File fileEntry : dir.listFiles()) {
			if(fileEntry.isDirectory()) return true;
		 }
	return false;
	}
